#!/usr/bin/gnuplot
################################################################################
# Script Gnuplot para gráfico de Speed-Up e Eficiência - MPI Odd-Even
################################################################################
...
if (!exists("output_file")) output_file = sprintf("plots/oddeven_speedup_%d.png", array_size)
...
if (strlen(baseline_time) == 0 || baseline_time eq "") {
    baseline_cmd = sprintf("LC_ALL=C awk -F',' 'NR>1 && $1==\"MPI_OddEven\" && $2==%d && $3==3 {print $6; exit}' %s", array_size, csv_file)
    baseline_time = system(baseline_cmd)
...
awk_cmd = sprintf("LC_ALL=C awk -F',' -v baseline=%s -v use_seq=%d '\
    ...
    }' %s | sort -n > plots/temp_oddeven_data.txt", baseline_time, use_seq_baseline, array_size, csv_file)
...
set title sprintf("Aplicação / Máquina (Odd-Even) - Array Size: %d", array_size) font 'Verdana,14'
...
set xrange [0:6]
set yrange [0:*]
set y2range [0:1.5]  # Até 1.5 para acomodar possíveis super-linear speedups
...
plot "plots/temp_oddeven_data.txt" using 1:3 with boxes axes x1y2 \
        lc rgb color_bars \
        fillstyle solid 0.5 \
        title "Eficiência", \
...
system("rm -f plots/temp_oddeven_data.txt")
