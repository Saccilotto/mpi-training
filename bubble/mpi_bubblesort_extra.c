/* IMPORTANT: Compile with -lm:
   mpicc mpi_bubblesort_extra.c.c -lm -o mpi_bubblesort_extra.c */
...
void bubble (int a[], int size, int temp[]);
...
//printf("Process %d has helper %d\n", my_rank, helper_rank);
      ...
      MPI_Recv (a + size / 2, size - size / 2, MPI_INT, helper_rank, tag,
		comm, &status);
      // bubble the two sorted sub-arrays through temp
      bubble (a, size, temp);
    }
  return;
}
...
void
bubblesort_serial (int a[], int size, int temp[])
{
  ...
  bubblesort_serial (a, size / 2, temp);
  bubblesort_serial (a + size / 2, size - size / 2, temp);
  // bubble the two sorted subarrays into a temp array
  bubble (a, size, temp);
}

void
bubble (int a[], int size, int temp[])
{
  int i1 = 0;
  int i2 = size / 2;
  int tempi = 0;
