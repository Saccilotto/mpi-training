...
# Odd-Even: todos (3, 7, 15, 31)
declare -A ODDEVEN_CONFIGS
ODDEVEN_CONFIGS["3"]="1"   # 3 processos, 1 nó
ODDEVEN_CONFIGS["7"]="1"   # 7 processos, 1 nó
ODDEVEN_CONFIGS["15"]="2"  # 15 processos, 2 nós
ODDEVEN_CONFIGS["31"]="4"  # 31 processos, 4 nós
...
run_test_oddeven() {
    ...
    if [ "$best_time" != "999999" ]; then
        ...
        echo "  ✗ Teste falhou - salvando com tempo -1"
        echo "${algorithm},${size},${procs},${nodes},0,-1,${ENVIRONMENT}" >> ${OUTPUT_FILE}
    fi
}
...
echo ""
echo "################################################################################"
echo "# Experimentos Atlantica: Quicksort vs Odd-Even"
echo "# Quicksort: 15 e 31 processos"
echo "# Odd-Even: 3, 7, 15 e 31 processos"
...
################################################################################
# FASE 2: ODD-EVEN (3, 7, 15 e 31 processos)
################################################################################

echo ""
echo "################################################################################"
echo "# FASE 2: MPI ODD-EVEN (3, 7, 15 e 31 processos)"
echo "################################################################################"
echo ""
...
for size in "${ARRAY_SIZES[@]}"; do
    echo ""
    echo "############################################################"
    echo "# ODD-EVEN - ARRAY SIZE: ${size}"
    echo "############################################################"

    for procs in "${!ODDEVEN_CONFIGS[@]}"; do
        nodes=${ODDEVEN_CONFIGS[$procs]}

        # Odd-Even não usa delta, usa função separada
...
cat > ${REPORT_FILE} << EOF
========================================================================
RELATÓRIO DE EXPERIMENTOS ATLANTICA - QUICKSORT VS ODD-EVEN
========================================================================
Data/Hora: ${TIMESTAMP}
Ambiente: ${ENVIRONMENT}
Quicksort: 15 (2 nós), 31 (4 nós)
Odd-Even: 3 (1 nó), 7 (1 nó), 15 (2 nós), 31 (4 nós)
...
for size in "${ARRAY_SIZES[@]}"; do
    ...
    # Odd-Even (todos os processos)
    for procs in 3 7 15 31; do
        oe_time=$(grep "MPI_OddEven,${size},${procs}" ${OUTPUT_FILE} | cut -d',' -f6)

        if [ -n "$oe_time" ] && [ "$oe_time" != "-1" ]; then
            echo "  ${procs} processos - Odd-Even: ${oe_time}s" >> ${REPORT_FILE}
        fi
    done

    echo "" >> ${REPORT_FILE}

    # Quicksort vs Odd-Even (apenas 15 e 31 onde ambos existem)
    for procs in 15 31; do
        qs_time=$(grep "MPI_Quicksort,${size},${procs}" ${OUTPUT_FILE} | cut -d',' -f6)
        oe_time=$(grep "MPI_OddEven,${size},${procs}" ${OUTPUT_FILE} | cut -d',' -f6)

        if [ -n "$qs_time" ] && [ -n "$oe_time" ] && [ "$qs_time" != "-1" ] && [ "$oe_time" != "-1" ]; then
...
for algo in "MPI_Quicksort" "MPI_OddEven"; do
    echo "" >> ${REPORT_FILE}
